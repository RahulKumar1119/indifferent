...
func TestIsValidVoice(t *testing.T) {
	validVoices := []string{"Joanna", "Matthew", "Ruth", "Danielle", "Aditi"}
	...
	invalidVoices := []string{"joanna", "MATTHEW", "Amy", "Brian", ""}
...
func TestSynthesize_AllRetriesExhausted(t *testing.T) {
	...
	_, err := svc.Synthesize(context.Background(), q, "Ruth")
	if err == nil {
...
func TestSynthesize_UsesNeuralEngine(t *testing.T) {
	...
	_, err := svc.Synthesize(context.Background(), q, "Danielle")
	if err != nil {
		t.Fatalf("Synthesize() unexpected error: %v", err)
	}
	...
	if capturedInput.Engine != types.EngineNeural {
		t.Errorf("expected neural engine, got %v", capturedInput.Engine)
	}
	if capturedInput.OutputFormat != types.OutputFormatMp3 {
		t.Errorf("expected mp3 output format, got %v", capturedInput.OutputFormat)
	}
	if capturedInput.VoiceId != types.VoiceId("Danielle") {
		t.Errorf("expected Danielle voice, got %v", capturedInput.VoiceId)
	}
}
